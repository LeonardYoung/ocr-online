...
def detect(save_img=False):
    ...
    # print(project_root)
    project_root = project_root + "\\yolov5-master\\"
    rootpath=project_root+"runs/detect/result/"
    ...
    for path, img, im0s, vid_cap in dataset:
        ...
        for i, det in enumerate(pred):  # detections per image
            ...
            #need change
            save_path = rootpath+str(p.name)
            # print(rootpath)
            # txt_path = rootpath+str(p.stem) + ('_%g' % dataset.frame if dataset.mode == 'video' else '')
            txt_path = rootpath + '1'
